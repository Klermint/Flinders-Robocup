...
cv2.namedWindow('ColourSegmentation')
# Low Boundary range of the HSV Colour Sliders.
cv2.createTrackbar('Hue_Low', 'ColourSegmentation', starting_colour[0], 255, nothing)
cv2.createTrackbar('Sat_Low', 'ColourSegmentation', starting_colour[1], 255, nothing)
cv2.createTrackbar('Val_Low', 'ColourSegmentation', starting_colour[2], 255, nothing)
# High Boundary range of the HSV Colour Sliders.
cv2.createTrackbar('Hue_High', 'ColourSegmentation', starting_colour[3], 255, nothing)
...
while True:
    ...
    Sat_Low = cv2.getTrackbarPos('Sat_Low', 'ColourSegmentation')
    Val_Low = cv2.getTrackbarPos('Val_Low', 'ColourSegmentation')
    Hue_High = cv2.getTrackbarPos('Hue_High', 'ColourSegmentation')
    ...
    # The Morph Close closes in small holes left in the masked image (Dilation)
    mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernal)
    # The Morph open is applied to remove noise from the background of the masked image (Erosion)
